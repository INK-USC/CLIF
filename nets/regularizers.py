...
class Regularizer(abc.ABC):
    def __init__(self, config, model, parallel_model, dataloaders, task_ids, output_dir, src_dir=None, prev_task=None):
        self.model = model
...
class MAS(Regularizer):
    ...
    def task_end_do(self):
        ...
        compute_importance(self.model, self.parallel_model, updater, self.dataloaders, task_ids=self.task_ids)
...
class EWC(Regularizer):
    ...
    def task_end_do(self):
        updater = Omega_update(self.model.parameters(), lr=0.0001, momentum=0.9)
        # config, model, parallel_model, updater, dataloaders, task_ids, loss_type)
        compute_importance(self.config, self.model, self.parallel_model, updater, self.dataloaders,
                           loss_type="ewc", task_ids=self.task_ids)
        accumulate_reg_params(self.model)
        self.save_reg_params()
...
def task_start_do(model, freeze_layers=[]):
    if not hasattr(model,"reg_params"):
        ...
        set_init_params(model, freeze_layers)
    else:
...
def set_init_params(model,freeze_layers=[], init_value=0):
    """initialize an omega for each parameter to zero or one"""
    for name, param in model.named_parameters():
        if not startswith_any(freeze_layers, name):
        #if not name in freeze_layers:
            ...
            init_val=param.data.clone()
            init_val=init_val.cuda()
            model.reg_params[param]['init_val'] = init_val
...
def compute_importance(config, model, parallel_model, updater, dataloaders, task_ids, loss_type):
    ...
    if loss_type == "l2":
        ...
        CELoss = CrossEntropyLoss(ignore_index=config.pad_token_id, reduction='mean')

    batch_size = None
    # Iterate over data.
    for dataloader, task_id in zip(dataloaders, task_ids):
        for step, batch in enumerate(tqdm(dataloader,desc='computing ewc importance')):
            if args.debug and step == 10:
                logger.warning('exit because of debugging')
                break
            ...
            if loss_type != "ewc":
                ...
                loss, _, ret_dict = model(cq_inputs, cq_attention_mask, ans_inputs, ans_attention_mask, labels=lb,
                                          is_training=True, task_id=task_id)
                #model_ret = model(**inputs)
                batch_size = cq_inputs.size(0)
                #loss.backward()
